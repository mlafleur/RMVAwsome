...
namespace RMV.Awesome.API.Models
{
    ...
    {
        static List<Branch> branchList = new List<Branch>();
        
        public async static Task<List<Branch>> GetAll()
        {
            ValidateList(); 
            await RefreshWaitTimes();
            return branchList.OrderBy(n=>n.DisplayName).ToList();
        }
        public async static Task<List<Branch>> GetAll(double deviceLatitude, double deviceLogitude)
        {
            ValidateList();
            await RefreshWaitTimes();
        ...
        private static async Task RefreshWaitTimes()
        {
            ...
            List<Branch> updatedBranchSet = new List<Branch>();
